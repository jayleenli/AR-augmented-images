...
public class AugmentedImageActivity extends AppCompatActivity implements GLSurfaceView.Renderer {
  ...
  private boolean setupAugmentedImageDatabase(Config config) {
    ...
    Log.i("setup aug image database","in setup aug image database" );
    if (useSingleImage) {
      Bitmap augmentedImageBitmap = loadAugmentedImageBitmap("default.jpg");
      Log.i("Loaded","loaded default" );
      Bitmap augmentedImageBitmap2 = loadAugmentedImageBitmap("globe.jpg");
      Log.i("Loaded","loaded other image" );
      if (augmentedImageBitmap == null) {
        return false;
      }
      Log.i("in one","in one setup aug image database" );
      augmentedImageDatabase = new AugmentedImageDatabase(session);
      //Add images to database
      augmentedImageDatabase.addImage("image_name", augmentedImageBitmap);
      augmentedImageDatabase.addImage("image_name2", augmentedImageBitmap2);
      // If the physical size of the image is known, you can instead use:
      //     augmentedImageDatabase.addImage("image_name", augmentedImageBitmap, widthInMeters);
      // This will improve the initial detection speed. ARCore will still actively estimate the
      // physical size of the image as it is viewed from multiple viewpoints.
    } else {
      Log.i("in two","in two setup aug image database" );
